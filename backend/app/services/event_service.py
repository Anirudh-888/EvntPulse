...
def create_event(db: Session, event_in: EventCreate, current_user: User) -> Event:
    ...
    # Organizer must own the club, or user is admin
...
def update_event(db: Session, event_id: int, event_in: EventUpdate, current_user: User) -> Event:
    ...
    if current_user.role != UserRole.ADMIN and event.club.owner_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only edit events belonging to your club"
        )
...
def change_event_status(db: Session, event_id: int, new_status: EventStatus, current_user: User) -> Event:
    ...
    if current_user.role != UserRole.ADMIN and event.club.owner_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only change status for your club's events"
        )
...
def delete_event(db: Session, event_id: int, current_user: User) -> bool:
    ...
    if current_user.role != UserRole.ADMIN and event.club.owner_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only delete events belonging to your club"
        )
